...
**여행하게** 는 게스트하우스 예약·동행을 주 서비스로 하는 중개 플랫폼을 목적으로 시작한 프로젝트입니다.
...
처음엔 **단일 서버**로 시작했지만, API와 도메인이 늘면서 **역할·성격에 따라 프로세스를 분리**하고, 나아가 추후 수평 확장을 고려해 각 서버를 **무상태**로 만들었습니다. 무상태가 불가능한 경우 — 소켓을 유지하는 message-server, 중복 실행을 막아야 하는 worker-server — 는 **Redis 어댑터**와 **분산 락**으로 일관성을 확보했습니다. → *(관련 글: [무상태 아키텍처](https://yeohaenghage.kr/backend/server_stateless/))*
...
| **upload-server** *(legacy)* | S3 **Presigned URL 발급** + 업로드 후 **사후처리 API** | 파일 트래픽이 서버를 우회 → [docs/upload.md](./docs/upload.md) |

### ⚙️ worker-server — "응답은 즉시, 처리는 뒤에서"
반응이 많은 작업(좋아요·조회수 등)의 사후 처리(DB 반영·랭킹 갱신·알림)를 라우터에서 직접 하지 않고 **큐에 넣고 즉시 응답**합니다(클라이언트는 낙관적 업데이트). 작업별 **중요도에 따라 재시도 정책**도 달리했습니다 — 좋아요 알림은 최소화, 예약 알림은 재시도+백오프 보강. → [docs/async-workers.md](./docs/async-workers.md)
...
- 설계 결정을 "감"이 아니라 **벤치마크로 증명**하는 습관을 들였다.
...
- **초기 출시 스코프를 너무 크게 잡았다.** [프론트엔드 저장소](https://github.com/kiwijuse/yeohaenghage)만 봐도 알 수 있듯, 페이지 수·기능 도메인이 초기 출시치고 지나치게 많았다. 지금 돌아보면 핵심 도메인(숙소 예약·동행 매칭)만으로 먼저 출시한 뒤, 도메인 확장·버그 수정·배포를 점진적으로 반복하는 편이 나았다.
- 소통은 자주 했지만, **정작 중요한 사전 정리·소통이 부족했다.** 백엔드에 비해 페이지가 많다 보니 프론트엔드 개발자 혼자 페이지 구현·설계·서버 연동을 전부 감당하기 어려웠고, 그래서 디자인이 끝난 페이지를 넘겨받아 연동 작업 대부분을 내가 도맡았다. 그런데 정작 이 과정에서 대략적인 연동 페이지 목록만 주고받은 탓에, 백엔드와의 네이밍 불일치나 페이지 구조 이해 부족(연동하려면 그 페이지의 구조·흐름부터 파악해야 한다)으로 불필요한 작업·지연이 발생했다.
...
- **"완성"보다 "출시"가 먼저다.** MVP는 핵심 도메인만으로 작게 잡고, 시장 반응을 보며 도메인을 넓히는 편이 모든 페이지를 완성해 한 번에 내놓는 것보다 리스크가 작다. 초기 스코프를 좁히는 것 자체가 하나의 설계 결정이다.
- **협업의 비용은 프론트–백엔드 각자의 구현이 아니라 그 사이의 인터페이스에 있다.**  — 네이밍 규약·API 계약·페이지 구조 등에 대한 공유된 이해를 먼저 문서 기반으로 합의하고 시작하는 것이 반복되는 소통 비용을 줄이는 가장 싼 방법이다.
