...
- **협업의 비용은 코드가 아니라 "경계"에 쌓인다.** 프론트–백엔드 연동에서 진짜 비용은 각자의 구현이 아니라 그 사이의 인터페이스 — 네이밍 규약·API 계약·페이지 구조의 공유된 이해 — 에 있다. 이 경계를 먼저 문서로 합의하고 시작하는 것이 반복되는 소통 비용을 줄이는 가장 싼 방법이다.
